<template>
  <v-container fluid>
    <v-expansion-panels focusable>
      <v-expansion-panel>
        <v-expansion-panel-header>BTC News</v-expansion-panel-header>
        <v-expansion-panel-content>
          <v-row class="my-5">
            <v-col
              md="4"
              v-for="(item, index) in newsBTC"
              :key="index"
            >
              <v-card class="mx-auto" max-width="344">
                <v-img :src="item.article.urlToImage" height="200px"></v-img>

                <v-card-title>
                  <p style="word-break: keep-all">
                    {{ item.article.title }}
                  </p>
                </v-card-title>

<<<<<<< HEAD
                
=======
                <v-card-subtitle>{{item.article.source.name}}</v-card-subtitle>
>>>>>>> 46e57f45e2d0dc3a26c11c549048caf9234232bd

                <v-card-actions>
                  <v-btn :href="item.article.url" target="_blank" color="orange lighten-2" text> Explore </v-btn>

                  <v-spacer></v-spacer>

                  <v-btn icon @click="item.show = !item.show">
                    <v-icon>{{
                      item.show ? "mdi-chevron-up" : "mdi-chevron-down"
                    }}</v-icon>
                  </v-btn>
                </v-card-actions>

                <v-expand-transition>
                  <div v-show="item.show">
                    <v-divider></v-divider>

                    <v-card-text>
<<<<<<< HEAD
                      {{item.description}}
=======
                      {{item.article.description}}
>>>>>>> 46e57f45e2d0dc3a26c11c549048caf9234232bd
                    </v-card-text>
                  </div>
                </v-expand-transition>
              </v-card>
            </v-col>
          </v-row>
        </v-expansion-panel-content>
      </v-expansion-panel>

      
    </v-expansion-panels>

      <v-expansion-panels focusable>
      <v-expansion-panel>
        <v-expansion-panel-header>ETH News</v-expansion-panel-header>
        <v-expansion-panel-content>
          <v-row class="my-5">
            <v-col
              md="4"
              v-for="(item, index) in newsETH"
              :key="index"
            >
              <v-card class="mx-auto" max-width="344">
                <v-img :src="item.article.urlToImage" lazy-src="data:image/png;base64," height="200px"></v-img>
                
                <v-card-title>
                  <p style="word-break: keep-all">
                    {{ item.article.title }}
                  </p>

                  
                </v-card-title>

<<<<<<< HEAD
               
=======
                <v-card-subtitle> {{item.article.source.name}}</v-card-subtitle>

>>>>>>> 46e57f45e2d0dc3a26c11c549048caf9234232bd
                <v-card-actions>
                  <v-btn :href="item.article.url" target="_blank" color="orange lighten-2" text> Explore </v-btn>

                  <v-spacer></v-spacer>

                  <v-btn icon @click="item.show = !item.show">
                    <v-icon>{{
                      show ? "mdi-chevron-up" : "mdi-chevron-down"
                    }}</v-icon>
                  </v-btn>
                </v-card-actions>

                <v-expand-transition>
                  <div v-show="item.show">
                    <v-divider></v-divider>

                    <v-card-text>
                       {{item.article.description}}
                    </v-card-text>
                  </div>
                </v-expand-transition>
              </v-card>
            </v-col>
          </v-row>
        </v-expansion-panel-content>
      </v-expansion-panel>

      
    </v-expansion-panels>


      <v-expansion-panels focusable>
      <v-expansion-panel>
        <v-expansion-panel-header>ADA News</v-expansion-panel-header>
        <v-expansion-panel-content>
          <v-row class="my-5">
            <v-col
              md="4"
              v-for="(item, index) in newsADA"
              :key="index"
            >
              <v-card class="mx-auto" max-width="344">
                <v-img :src="item.article.urlToImage" height="200px"></v-img>

                <v-card-title>
                  <p style="word-break: keep-all">
                    {{ item.article.title }}
                  </p>
                </v-card-title>

                <v-card-subtitle> {{item.article.author}} </v-card-subtitle>

                <v-card-actions>
                  <v-btn :href="item.article.url" target="_blank" color="orange lighten-2" text> Explore </v-btn>

                  <v-spacer></v-spacer>

                  <v-btn icon @click="item.show = !item.show">
                    <v-icon>{{
                      item.show ? "mdi-chevron-up" : "mdi-chevron-down"
                    }}</v-icon>
                  </v-btn>
                </v-card-actions>

                <v-expand-transition>
                  <div v-show="item.show">
                    <v-divider></v-divider>

                    <v-card-text>
                      {{item.article.description}}
                    </v-card-text>
                  </div>
                </v-expand-transition>
              </v-card>
            </v-col>
          </v-row>
        </v-expansion-panel-content>
      </v-expansion-panel>

      
    </v-expansion-panels>


  </v-container>
</template>

<script>
import axios from "axios";
export default {
  name: "News",
  data() {
    return {
      newsBTC: [],
      newsETH: [],
      newsADA: [],
      show: false,
    };
  },
  created() {
    let todaysDate=`${new Date().getFullYear()}-${new Date().getMonth()+1}-${new Date().getDate()-1}`
    
    axios
      .get(
        `https://newsapi.org/v2/everything?q=bitcoin&from=${todaysDate}&sortBy=publishedAt&domains=coindesk.com,thestonkmarket.com&language=en&apiKey=d547e4173b4a4a838565cf33fa069dd8`
      )
      .then((res) => 
        {
          res.data.articles.forEach(article => {
            this.newsBTC.push({article: article, show: false});
          });
        })
      .catch((err) => console.log(err));
    axios
      .get(
        `https://newsapi.org/v2/everything?q=ethereum&from=${todaysDate}&sortBy=publishedAt&domains=thestreet.com,bloomberg.com,coindesk.com,coinbase.com&language=en&apiKey=d547e4173b4a4a838565cf33fa069dd8`
      )
      .then((res) => 
        {
          res.data.articles.forEach(article => {
            this.newsETH.push({article: article, show: false});
          });
        })
      .catch((err) => console.log(err));
    axios
      .get(
        `https://newsapi.org/v2/everything?q=cardano&from=${todaysDate}&sortBy=publishedAt&domains=fool.com,coindesk.com,coinbase.com&language=en&apiKey=d547e4173b4a4a838565cf33fa069dd8`
      )
      .then((res) => 
        {
          res.data.articles.forEach(article => {
            this.newsADA.push({article: article, show: false});
          });
        })
      .catch((err) => console.log(err));
  },
};
</script>

<style>
</style>
